...
class Compiler():
    ...
    def compile_pug(self):
        ...
        for pug_files in os.listdir(os.getcwd() + "/src"):
            if ".pug" in pug_files:
        ...
        for page in target_pages:
            try:
                output = subprocess.check_output(["./node_modules/.bin/pug", os.getcwd() + "/src/" + page, "--out", "./src/dist/"])
            except:
    ...
    def compile_sass(self):
        Log("Compiling sass...")
        try:
            output = subprocess.check_output(["./node_modules/.bin/node-sass", "--output-style", "compressed", "./src/styles/site.scss", "./src/dist/site.css"])
        except:
    ...
    def create_js(self):
        ...
        try:
            scripts = os.listdir("./src/scripts/")
        except:
            Log("Failed to get JavaScripts!", withError = True)
        ...
        for js in scripts:
            if ".js" in js:
                out = subprocess.check_output(["./node_modules/.bin/javascript-obfuscator", "./src/scripts/" + js , "--selfDefending", "false", "--output", "tmp.js"])
                with open("tmp.js", "r") as f:
                    script_string += f.read()
                    os.remove("tmp.js")


        os.mkdir("./src/dist/script/")
        with open("./src/dist/script/site.js", "w") as write_target:
            write_target.write(script_string)

        shutil.copy2("./src/dist/script/site.js", "./src/dist/site.js")


    def create_assets(self):   
        Log("Creating assets pack...")
        try:
            dir_util.copy_tree('./src/assets/', './src/dist/assets/')
        except:
...
class DebugUtils():
    def open_in_chrome(self, URL = os.curdir + "/src/dist/index.html"):
    ...
    def open_in_safari(self):
        ...
        subprocess.call(["open", "-a", "Safari", os.curdir + "/src/dist/index.html"])

    def open_in_firefox(self):
        Log("Opening in Firefox...")
        subprocess.call(["open", "-a", "Firefox", os.curdir + "/src/dist/index.html"])
